DELIMITER $$
CREATE DEFINER=`id1424488_membre`@`localhost` PROCEDURE `AddArticle`(IN `ID` INT(50), IN `Nom` VARCHAR(50), IN `Descr` VARCHAR(50), IN `Prix` INT(50), IN `IdT` INT(50), IN `Den` VARCHAR(50))
    MODIFIES SQL DATA
...
DELIMITER $$
CREATE DEFINER=`id1424488_membre`@`localhost` PROCEDURE `AddPhoto`(IN `Nom` VARCHAR(50), IN `ID` INT)
    MODIFIES SQL DATA
INSERT INTO photo (Nom_Photo, Moderation, Id_Activite) VALUES (Nom, 0, ID)$$
DELIMITER ;

DELIMITER $$
CREATE DEFINER=`id1424488_membre`@`localhost` PROCEDURE `DeleteImage`(IN `ID` INT)
    MODIFIES SQL DATA
DELETE FROM commentaire_photo WHERE Id_Photo = ID$$
DELIMITER ;

DELIMITER $$
CREATE DEFINER=`id1424488_membre`@`localhost` PROCEDURE `DeleteAvatar`(IN `ID` INT)
    MODIFIES SQL DATA
UPDATE `utilisateur` SET `Avatar` = 'Images/avatar.jpg' WHERE `Id_utilisateur`= ID$$
DELIMITER ;

DELIMITER $$
CREATE DEFINER=`id1424488_membre`@`localhost` PROCEDURE `DeleteArticle`(IN `ID` INT)
    MODIFIES SQL DATA
DELETE FROM article WHERE Id_Article = ID$$
DELIMITER ;

DELIMITER $$
CREATE DEFINER=`id1424488_membre`@`localhost` PROCEDURE `DeleteCom`(IN `ID` INT)
    MODIFIES SQL DATA
DELETE FROM photo WHERE Id_Photo = ID$$
DELIMITER ;

DELIMITER $$
CREATE DEFINER=`id1424488_membre`@`localhost` PROCEDURE `DeleteLike`(IN `ID` INT)
    NO SQL
DELETE FROM like_photo WHERE Id_Photo = ID$$
DELIMITER ;

DELIMITER $$
CREATE DEFINER=`id1424488_membre`@`localhost` PROCEDURE `GetActivites`()
    READS SQL DATA
SELECT Id_Activite AS ID, Nom_Activite AS Nom, Date_Activite AS DateA, photo_Activites AS Image FROM activites$$
DELIMITER ;

DELIMITER $$
CREATE DEFINER=`id1424488_membre`@`localhost` PROCEDURE `GetAllUser`()
...
DELIMITER $$
CREATE DEFINER=`id1424488_membre`@`localhost` PROCEDURE `GetAllUserInfo`()
    READS SQL DATA
...
DELIMITER $$
CREATE DEFINER=`id1424488_membre`@`localhost` PROCEDURE `GetArtAdmin`()
    READS SQL DATA
SELECT Nom_Article AS Image, Denomination AS Nom, Prix_Article AS Article, Id_Article AS ID FROM article$$
DELIMITER ;

DELIMITER $$
CREATE DEFINER=`id1424488_membre`@`localhost` PROCEDURE `GetArticles`()
    READS SQL DATA
SELECT Nom_Article AS Image, Denomination AS Nom, Id_Article AS ID FROM article$$
DELIMITER ;

DELIMITER $$
CREATE DEFINER=`id1424488_membre`@`localhost` PROCEDURE `GetAvatar`()
    READS SQL DATA
...
DELIMITER $$
CREATE DEFINER=`id1424488_membre`@`localhost` PROCEDURE `GetCommentaires`(IN `VID` INT)
    READS SQL DATA
...
DELIMITER $$
CREATE DEFINER=`id1424488_membre`@`localhost` PROCEDURE `GetDetailA`(IN `VID` INT)
    READS SQL DATA
...
DELIMITER $$
CREATE DEFINER=`id1424488_membre`@`localhost` PROCEDURE `GetDetailP`(IN `VID` INT)
    READS SQL DATA
...
DELIMITER $$
CREATE DEFINER=`id1424488_membre`@`localhost` PROCEDURE `GetDetailPhoto`(IN `VID` INT)
    READS SQL DATA
SELECT Nom_Photo AS Image FROM photo WHERE Id_Photo = VID$$
DELIMITER ;

DELIMITER $$
CREATE DEFINER=`id1424488_membre`@`localhost` PROCEDURE `GetFonction`()
    READS SQL DATA
...
DELIMITER $$
CREATE DEFINER=`id1424488_membre`@`localhost` PROCEDURE `GetUserFonction`(IN `Vmail` VARCHAR(50))
    READS SQL DATA
SELECT Id_Fonction AS Funct FROM utilisateur WHERE Mail = Vmail$$
DELIMITER ;

DELIMITER $$
CREATE DEFINER=`id1424488_membre`@`localhost` PROCEDURE `GetIActivites`()
    READS SQL DATA
SELECT Id_Idee_Activite AS ID, Nom_Idee_Activite AS Nom, Images_I_A AS Image FROM idees_activites$$
DELIMITER ;

DELIMITER $$
CREATE DEFINER=`id1424488_membre`@`localhost` PROCEDURE `GetLikes`(IN `VID` INT)
    READS SQL DATA
SELECT COUNT(Id_like) AS Likes FROM like_photo WHERE Id_Photo = VID$$
DELIMITER ;

DELIMITER $$
CREATE DEFINER=`id1424488_membre`@`localhost` PROCEDURE `GetModerationActivites`()
    READS SQL DATA
...
DELIMITER $$
CREATE DEFINER=`id1424488_membre`@`localhost` PROCEDURE `GetModerationPhotos`()
    READS SQL DATA
SELECT Nom_Photo AS Image, Moderation, Id_Photo AS IDv, Id_Photo AS IDs FROM photo ORDER BY Moderation$$
DELIMITER ;

DELIMITER $$
CREATE DEFINER=`id1424488_membre`@`localhost` PROCEDURE `GetPhotos`()
    READS SQL DATA
SELECT Nom_Photo AS Image FROM photo WHERE Id_Activite = IDA AND Moderation = 1$$
DELIMITER ;

DELIMITER $$
CREATE DEFINER=`id1424488_membre`@`localhost` PROCEDURE `GetStock`(IN `VID` INT)
    READS SQL DATA
...
DELIMITER $$
CREATE DEFINER=`id1424488_membre`@`localhost` PROCEDURE `GetUserID`(IN `Vmail` VARCHAR(50))
    READS SQL DATA
SELECT Id_utilisateur AS ID FROM utilisateur WHERE Mail = Vmail$$
DELIMITER ;

DELIMITER $$
CREATE DEFINER=`id1424488_membre`@`localhost` PROCEDURE `GetUserInfo`(IN `Vmail` VARCHAR(50))
    READS SQL DATA
SELECT Nom_Utilisateur AS Nom, Prenom_Utilisateur AS Prenom, Avatar FROM utilisateur WHERE Mail = Vmail$$
DELIMITER ;

DELIMITER $$
CREATE DEFINER=`id1424488_membre`@`localhost` PROCEDURE `GetUserLike`(IN `VID` INT)
    READS SQL DATA
...
DELIMITER $$
CREATE DEFINER=`id1424488_membre`@`localhost` PROCEDURE `Inscription`(IN `mdp` VARCHAR(50), IN `nom` TEXT, IN `prenom` TEXT, IN `mail` VARCHAR(50), IN `fonction` INT(5) UNSIGNED)
    MODIFIES SQL DATA
INSERT INTO utilisateur (Mdp, Nom_Utilisateur, Prenom_Utilisateur, Mail, Id_Fonction) VALUES (SHA1(mdp), nom, prenom, mail, fonction)$$
DELIMITER ;

DELIMITER $$
CREATE DEFINER=`id1424488_membre`@`localhost` PROCEDURE `Like`(IN `IDU` INT, IN `IDP` INT)
    MODIFIES SQL DATA
INSERT INTO like_photo (Id_utilisateur, Id_Photo) VALUES (IDU, IDP)$$
DELIMITER ;

DELIMITER $$
CREATE DEFINER=`id1424488_membre`@`localhost` PROCEDURE `Login`(IN `Vmdp` VARCHAR(50), IN `Vmail` VARCHAR(50))
    READS SQL DATA
SELECT Id_utilisateur FROM utilisateur WHERE Mdp = SHA1(Vmdp) AND Mail = Vmail$$
DELIMITER ;

DELIMITER $$
CREATE DEFINER=`id1424488_membre`@`localhost` PROCEDURE `SetAvatar`(IN `Image` VARCHAR(50), IN `Vmail` INT)
    MODIFIES SQL DATA
UPDATE utilisateur
SET Avatar = Image
WHERE Mail = Vmail$$
...
DELIMITER $$
CREATE DEFINER=`id1424488_membre`@`localhost` PROCEDURE `UpdateInfos`(IN `nom` TEXT, IN `prenom` TEXT, IN `dateN` VARCHAR(50), IN `adresse` VARCHAR(50), IN `codeP` INT, IN `Ville` VARCHAR(50))
    MODIFIES SQL DATA
UPDATE utilisateur
...
DELIMITER $$
CREATE DEFINER=`id1424488_membre`@`localhost` PROCEDURE `ValideImage`(IN `ID` INT)
    MODIFIES SQL DATA
UPDATE `photo` SET `Moderation` = '1' WHERE `photo`.`Id_Photo` = ID$$
DELIMITER ;

DELIMITER $$
CREATE DEFINER=`id1424488_membre`@`localhost` PROCEDURE `VoteA`(IN `IDA` INT, IN `IDU` INT)
    MODIFIES SQL DATA
