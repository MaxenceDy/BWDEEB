DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `AddActivite`(IN `nom` VARCHAR(50), IN `des` VARCHAR(50), IN `prix` INT, IN `photo` VARCHAR(80), IN `val` INT)
    MODIFIES SQL DATA
...
DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `AddPhoto`(IN `Nom` VARCHAR(255), IN `m` INT, IN `ID` INT)
    MODIFIES SQL DATA
...
DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `AddPropDate`(IN `dateIA` DATETIME, IN `ID` INT)
    MODIFIES SQL DATA
...
DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `GetActiID`(IN `nom` VARCHAR(50))
    READS SQL DATA
